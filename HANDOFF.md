...
## 8. 남은 할 일 (우선순위순)
- [ ] **(신규, 최우선) `push_tokens` 테이블 실제 DB에 생성** — `supabase/schema.sql`의 새 부분을 Supabase SQL Editor에서 실행 (자세한 내용 9-1절)
- [ ] **(신규, 최우선) 네이버 뉴스 검색 API 키 발급 후 `supabase secrets set NAVER_CLIENT_ID/NAVER_CLIENT_SECRET`** (자세한 내용 9-1절)
...
1. **`push_tokens` 테이블이 실제 Supabase DB에 없음.** `supabase/schema.sql`은 로컬 파일만 수정됐고 아직 DB에 적용 안 됨 (`curl .../rest/v1/push_tokens` → `PGRST205 Could not find the table`). → Supabase 대시보드 SQL Editor에서 `schema.sql`의 새 `push_tokens` 관련 부분을 실행해야 함. 이거 전엔 `registerPushToken()`이 조용히 실패(catch돼서 앱은 안 죽음)하고 `notify-new-article`은 토큰 0개로 응답함.
2. **네이버 뉴스 API 시크릿이 Supabase에 설정 안 됨.** `get-breaking-news` 호출 시 401 (`curl .../functions/v1/get-breaking-news` → `{"error":"네이버 API 응답 오류: 401"}`). 네이버 개발자센터(https://developers.naver.com)에서 애플리케이션 등록(검색 API) 후 Client ID/Secret 발급받아 `supabase secrets set NAVER_CLIENT_ID=... NAVER_CLIENT_SECRET=...` 실행 필요. 이거 전엔 앱이 조용히 기존 placeholder 속보로 폴백하므로 겉으로는 안 터짐.
