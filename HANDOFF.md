...
- [ ] ⚠️ **빌드 시 Supabase 환경변수 주입**: `.env`(EXPO_PUBLIC_SUPABASE_URL/ANON_KEY)는 gitignore라 클론된 PC엔 없음. 로컬 빌드는 `.env` 재생성, **EAS 빌드는 EAS env var로 설정** 안 하면 앱이 SEED/placeholder로 폴백(원격 글·속보·푸시토큰 안 됨)
