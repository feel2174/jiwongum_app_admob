...
- 어드민 페이지(`admin/index.html`)는 로컬(`python3 -m http.server`)로 열어서 로그인 확인함.

**아직 안 된 것 (다음 작업자가 이어갈 부분)**:
- 어드민 "새 글 가져오기" 탭에서 실제로 워드프레스 글을 선택→태깅→발행까지 해보고, 그 글이 앱(dev client)에서 실제로 보이는지 end-to-end 1회 확인 (Edge Function/DB/RLS는 개별적으로 검증됐지만 어드민 UI를 통한 전체 흐름은 아직 안 눌러봄).
- 어드민 페이지는 로컬 임시 서버로만 열어봤음. 상시 접근하려면 Vercel/GitHub Pages 등에 정식 배포 필요.
...
- [ ] 어드민 페이지 정식 배포 (Vercel 등, 지금은 로컬에서만 열어봄)
...
- [x] 콘텐츠 관리 어드민 도구 + Supabase 연동 (스키마·RLS·마이그레이션·앱 리팩터) — 위 "콘텐츠 파이프라인" 절 참고
