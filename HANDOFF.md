...
## 8. 남은 할 일 (우선순위순)
- [x] ✅ **`push_tokens` 테이블 생성 완료** (2026-07-12, Supabase SQL Editor 실행) — 실제 토큰 등록 end-to-end는 재빌드 후 검증
- [x] ✅ **EAS 빌드 Supabase 환경변수 주입 완료** — `eas.json`의 development/preview/production 각 `env`에 EXPO_PUBLIC_SUPABASE_URL/ANON_KEY(publishable 키, 공개 안전) 박음. 로컬 `expo run`만 별도 `.env` 필요.
- [x] ✅ **푸시 알림 권한/아이콘 세팅 완료** (2026-07-12) — POST_NOTIFICATIONS 권한, 알림 아이콘(흰 ₩), opt-in 토글, 토큰 로깅. prebuild 매니페스트로 검증. 테스트 절차는 `PUSH_TESTING.md`.
- [ ] ⚠️ **(푸시 전제) FCM 자격증명 설정** — Android 원격 푸시 전송엔 Firebase FCM V1 자격증명 필요. `eas credentials` → Android → FCM V1에 서비스 계정 키 업로드. 안 하면 토큰은 나와도 알림 미도착. (상세: `PUSH_TESTING.md`)
- [ ] **(현재 단계) dev client 재빌드**: `eas build --profile development --platform android` → APK 설치 → `npx expo start --dev-client` → end-to-end 검증(속보·푸시·어드민 발행)
- [x] ✅ **네이버 뉴스 API 키 발급 + Supabase 시크릿 설정 완료** (2026-07-12) — `get-breaking-news`가 실제 최신 뉴스 반환하는 것 curl로 검증됨
- [ ] **(신규) 위 2개 끝난 후 dev client 재빌드 + 푸시알림/속보 end-to-end 실기기 검증**
...
## 9-1. 이번 세션 작업내용 (2026-07-11) — 실시간 푸시알림 + 실제 속보 API

이전 HANDOFF의 "남은 할 일"에 있던 "(선택) 원격 푸시(FCM) 구축"과 "속보 영역"을 실제 뉴스로 채우는 작업을 시작함. **코드는 다 짜여있고 Edge Function도 배포됐지만, 아래 2가지 외부 설정이 안 끝나서 아직 실동작 안 함.**

### 추가된 것
- **속보 = 실제 뉴스**: `src/lib/breakingNews.js`가 신규 Edge Function `get-breaking-news`(네이버 뉴스 검색 API 프록시, `verify_jwt:false`)를 호출. 실패 시 `null` 반환 → `store.js`가 기존 `reco.js`의 `news(articles)` placeholder로 자동 폴백(화면엔 항상 뭔가 뜸, 조용히 대체).
- **실시간 푸시 알림(전체 브로드캐스트, 상황별 타겟팅 없음)**:
  - `src/lib/push.js`: `registerPushToken()`(Expo push token 발급 → Supabase `push_tokens` upsert), `unregisterPushToken()`, `syncPushTokenIfPermitted()`(앱 시작 시 이미 허용된 기기는 조용히 토큰 갱신, `App.js`에 연결).
  - `SettingsScreen.js`: "새 글 알림" 토글 on→`registerPushToken()`(권한요청 포함), off→`unregisterPushToken()`.
  - `admin/index.html`: 글 저장 시 발행(`saveImport` published=true) 또는 발행전환(`togglePublished`) 직후 신규 Edge Function `notify-new-article` 호출 → `push_tokens` 전체에 Expo Push API로 발송.
  - `supabase/schema.sql`에 `push_tokens` 테이블 + RLS 추가(anon은 insert/delete만, select 불가 — 대량 수집 방지).
  - `app.json`에 `expo-notifications` 플러그인 블록(아이콘 컬러) 추가, `package.json`에 `expo-constants` 의존성 추가(push token 발급에 projectId 필요).

### ⚠️ 아직 실동작 안 함 (다음 작업자가 반드시 먼저 확인/처리) — 직접 curl로 검증한 결과:
1. ✅ **[해결됨 2026-07-12] `push_tokens` 테이블 생성 완료** (Supabase SQL Editor에서 schema.sql의 push_tokens 부분 실행). 실제 토큰 등록은 재빌드 후 앱에서 알림 켜서 확인 예정.
2. ✅ **[해결됨 2026-07-12] 네이버 뉴스 API 시크릿 설정 완료.** 네이버 검색 API Client ID/Secret을 Supabase에 설정했고, `get-breaking-news`가 실제 최신 뉴스를 정상 반환하는 것 curl로 확인함(더 이상 401 아님).
3. Edge Function 배포 자체는 완료됨(`npx supabase functions list` → `get-breaking-news`/`notify-new-article` 둘 다 `ACTIVE`).
4. **네이티브 리빌드 필요**: `app.json`에 `expo-notifications` 플러그인 블록을 새로 추가했고 `expo-constants`가 새 의존성으로 들어감 → 기존 dev build로는 반영 안 될 수 있음, `npx expo prebuild`/dev client 재빌드 후 테스트할 것.
5. End-to-end 미검증: 위 1·2 해결 후 "설정에서 알림 켜기 → 어드민에서 글 발행 → 실제 기기에 푸시 도착" 그리고 "홈 화면 속보 섹션에 실제 네이버 뉴스 표시" 둘 다 한 번도 실기기로 확인 안 해봄.
